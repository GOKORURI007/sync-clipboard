...
class ClipboardMonitor:
    ...
    async def start_monitoring(self) -> None:
        """开始监听剪贴板变化"""
        try:
            self.current_content = str(self._safe_clipboard_get())
        ...
        while True:
            ...
            try:
                ...
                if (current_content != self.current_content and 
                    ...
                    self._is_content_changed(current_content)):
    ...
    def _is_content_changed(self, new_content: str) -> bool:
        ...
        # 标准化内容：去除首尾空白，统一换行符
        normalized_current = self.current_content.strip().replace('\r\n', '\n').replace('\r', '\n')
    ...
    async def update_clipboard(self, content: str) -> None:
        """更新本地剪贴板（防回环）"""
        if content != self.current_content and content.strip():
            # 设置同步状态标志，防止触发新的同步循环
            self.is_syncing = True
            try:
                self._safe_clipboard_set(content)
                self.current_content = content
                self.logger.debug(f"已更新剪贴板内容: {content[:50]}{'...' if len(content) > 50 else ''}")
                # 短暂等待确保剪贴板更新完成
