...
class SyncClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # 发送hello消息
            hello_message = Message(type='client_hello', sender_id=self.hostname)
    ...
    async def send_clipboard_update(self, content: str) -> None:
        """发送剪贴板更新到服务器"""
        if self.websocket:
            message = Message(type='clipboard_update', sender_id=self.hostname, content=content)
            try:
    ...
    async def handle_server_message(self, message: Message) -> None:
        ...
        if message.type == 'clipboard_update':
            ...
            if sender == self.hostname:
                self.logger.debug(
                    f'忽略自己发送的消息: {content[:50]}{"..." if len(content) > 50 else ""}'
                )
                return
    ...
    async def attempt_reconnect(self) -> bool:
        ...
        self.logger.info(
            f'连接断开，{wait_time}秒后尝试重新连接... (第 {self.retry_count}/{self.max_retries} 次)'
        )
