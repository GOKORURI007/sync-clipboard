...
class SyncServer:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            raise ClipboardConnectionError(f'无法在 {self.host}:{self.port} 启动服务器: {e}')
    ...
    async def _start_websocket_server(self) -> None:
        ...
        try:
            ...
            self.logger.info('服务器已启动，等待客户端连接...')
            await self.server.wait_closed()
        except OSError as e:
    ...
    async def _handle_clipboard_update(
        self, message: Message, sender_websocket: ServerConnection
    ) -> None:
        """处理剪贴板更新消息"""
    ...
    async def broadcast_clipboard_update(
        self, content: str, sender_websocket: Optional[ServerConnection] = None
    ) -> None:
        """广播剪贴板更新给所有客户端（排除发送方）"""
        if not self.clients:
            return
        ...
        message = Message(type='clipboard_update', sender_id=sender_hostname, content=content)

        # 给所有客户端（除了发送者）发送更新
        disconnected_clients = []
        for client_websocket, client_hostname in self.clients.items():
            # 排除发送方：既要排除websocket连接，也要排除相同主机名的客户端
            if client_websocket != sender_websocket and client_hostname != sender_hostname:
                try:
                    ...
                    self.logger.error(
                        f'向客户端 {client_hostname} 广播时发生未知错误: {e}', exc_info=True
                    )
                    disconnected_clients.append(client_websocket)
