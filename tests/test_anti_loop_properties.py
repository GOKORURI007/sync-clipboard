...
class TestAntiLoopProperties:
        ...
        async def _test():
            with mock_clipboard() as (mock_paste, mock_copy):
                ...
                assert len(sender_ws.sent_messages) == 0, (
                    f'Sender {sender_hostname} should not receive their own message'
                )
                ...
                for hostname, ws in client_websockets.items():
                    if hostname != sender_hostname:
                        assert len(ws.sent_messages) == 1, (
                            f'Client {hostname} should receive the broadcast'
                        )
    ...
    @given(content_variations=st.lists(text(min_size=0, max_size=100), min_size=2, max_size=10))
    @settings(
        max_examples=100, deadline=5000, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
            ...
            with mock_clipboard() as (mock_paste, mock_copy):
                ...
                for content in content_variations:
                    ...
                    if not monitor.is_syncing:
                        ...
                        if (
                            current_clipboard != monitor.cached_content
                            and current_clipboard.strip()
                            and monitor._is_content_changed(current_clipboard)
                        ):
                ...
                for i in range(1, len(callback_calls)):
                    prev_normalized = (
                        callback_calls[i - 1].strip().replace('\r\n', '\n').replace('\r', '\n')
                    )
                    curr_normalized = (
                        callback_calls[i].strip().replace('\r\n', '\n').replace('\r', '\n')
                    )
                ...
                for callback_content in callback_calls:
                    ...
                    assert normalized, (
                        f"Empty content should not trigger callback: '{callback_content}'"
                    )
        ...
        async def _test():
            with mock_clipboard() as (mock_paste, mock_copy):
                ...
                other_message = Message(
                    type='clipboard_update', sender_id='other_host', content=content
                )
